...
identity='rescale_then_dust_stef' #wide_prior
...
x,y,emulator=mcmc_functions.build_emulator(home_mop,suite,prof) #y is (19,nsamp)

pos = [theta0 + 5e-3*np.random.randn(ndim) for i in range(nwalkers)]
with Pool() as pool:
    prob_model=mcmc_functions.lnprob
    ...
    np.savetxt('./mcmc_test/samples_total_'+suite+'_'+prof+'_run'+run+'_'+identity+'.txt',samples)
    samp_reshaped = sampler.chain[:, :, :].reshape((-1, ndim))
    cut=300
    
    samples_burn = sampler.chain[:, cut:, :].reshape((-1, ndim))
    np.savetxt('./mcmc_test/samples_burn_'+suite+'_'+prof+'_run'+run+'_'+identity+'.txt',samples_burn)
    print("#Mean acceptance fraction: {0:.3f}".format(np.mean(sampler.acceptance_fraction)))

    #logprob
    logprob=sampler.get_log_prob()
    np.savetxt('./mcmc_test/logprob_total_'+suite+'_'+prof+'_run'+run+'_'+identity+'.txt',logprob)
    logprob_burn=logprob[cut:,:]
    np.savetxt('./mcmc_test/logprob_burn_'+suite+'_'+prof+'_run'+run+'_'+identity+'.txt',logprob_burn)
    ...
    plt.savefig('./mcmc_test/converge_combined_'+suite+'_'+prof+'_run'+run+'_'+identity+'.png')
    plt.close()
