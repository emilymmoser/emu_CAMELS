...
mass_range_latex=np.array(['$11 \leq \log_{10}(M_{200c}/M_\odot) \leq 11.5$','$11.5 \leq \log_{10}(M_{200c}/M_\odot) \leq 12$'])
...
def set_suite(suite):
    txt_file = 'CosmoAstroSeed_'+suite+'.txt'
...
def build_emulator_CMASS(home,suite,prof,func_str):
    ...
    y=np.transpose(y)
    print(np.shape(y),np.shape(samples))
    emulator=ostrich.emulate.PcaEmulator.create_from_data(
        samples,
        y,
        ostrich.interpolate.RbfInterpolator,
...
def compute_pm_profiles_CMASS(vary_str,A_emu,delta_thet,emulator,x,omega_m,sigma_8,z,M):
    if vary_str == 'ASN1':
        params_plus=[[0.3,0.8,A_emu+delta_thet,1.0,1.0,1.0,z,M]]
        params_minus=[[0.3,0.8,A_emu-delta_thet,1.0,1.0,1.0,z,M]]
    elif vary_str == 'AAGN1':
        params_plus=[[0.3,0.8,1.0,A_emu+delta_thet,1.0,1.0,z,M]]
        params_minus=[[0.3,0.8,1.0,A_emu-delta_thet,1.0,1.0,z,M]]
    elif vary_str == 'ASN2':
        params_plus=[[0.3,0.8,1.0,1.0,A_emu+delta_thet,1.0,z,M]]
        params_minus=[[0.3,0.8,1.0,1.0,A_emu-delta_thet,1.0,z,M]]
    elif vary_str == 'AAGN2':
        params_plus=[[0.3,0.8,1.0,1.0,1.0,A_emu+delta_thet,z,M]]
        params_minus=[[0.3,0.8,1.0,1.0,1.0,A_emu-delta_thet,z,M]]
