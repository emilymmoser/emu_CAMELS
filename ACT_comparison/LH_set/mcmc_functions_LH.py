...
def set_suite(suite):
    txt_file = 'CosmoAstroSeed_'+suite+'_reduced.txt' #reduced is cutting some of the sims out due to radial limits
...
#-------------- mcmc functions
rescaling_factor_dict={'rho_mean':np.array([1.29434376,1.50186486,1.68729486,1.7872668,1.82233295,1.80844895,1.79453689,1.79890591,1.7996796]),
...
def add_dust(tsz_profile): #tsz in muK*sr
    ...
    #remove inner radial bin
    #dust_profile=np.array([0.60664831,1.10251209,1.38862518,1.39781291,1.13080406,0.65623086,0.11062901,0.10156234])
...
def lnlike(theta,thta_arc,proj,cov,x,prof,emulator):
    ...
    model_proj=project_profiles(prof,thta_arc,z_set,nu,fBeamF_150,respT_150,x,profile)
...
def load_ACT_data(home,prof):
    ...
    #thta_act,act_data,act_err=np.loadtxt(home+'diskring_'+file_string+'_varweight_measured_remove_inner_bin.txt',usecols=(0,1,2),unpack=True)

    cov=np.loadtxt(home+'cov_diskring_'+file_string+'_varweight_bootstrap.txt')
    #cov=np.loadtxt(home+'cov_diskring_'+file_string+'_varweight_bootstrap_remove_inner_bin.txt')
