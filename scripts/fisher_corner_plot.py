...
derivatives_rho=np.genfromtxt('/home/cemoser/Repositories/emu_CAMELS/tests/derivatives_array_'+suite+'_rho_mean.txt')    
derivatives_pth=np.genfromtxt('/home/cemoser/Repositories/emu_CAMELS/tests/derivatives_array_'+suite+'_pth_mean.txt')
...
samp_rho=MCSamples(samples=chain_rho,names=vary_arr,labels=vary_arr,label=r'$\rho$')
samp_pth=MCSamples(samples=chain_pth,names=vary_arr,labels=vary_arr,label=r'$P_{th}$')
samp_combined=MCSamples(samples=chain_combined,names=vary_arr,labels=vary_arr,label=r'$\rho + P_{th}$')
g=plots.getSubplotPlotter()
#samp.updateSettings({'contours':[0.68]})
g.settings.figure_legend_frame=False
g.triangle_plot([samp_rho,samp_pth,samp_combined],filled=True)
plt.savefig('/home/cemoser/Repositories/emu_CAMELS/tests/corner_'+suite+'_combined.png',bbox_inches='tight')
