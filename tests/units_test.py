...
x,y,errup,errlow,stddev=fs.load_profiles_3D(usecols,home,suite,sims,snap,mass_str)
...
np.savetxt('/home/cemoser/Repositories/ostrich/Tests/Figures/emulator3D/units_test/errs_drop1_'+suite+'_'+prof+'_'+vary_str+'.txt',errs_drop_1)
np.savetxt('/home/cemoser/Repositories/ostrich/Tests/Figures/emulator3D/units_test/emulated_drop1_'+suite+'_'+prof+'_'+vary_str+'.txt',emulated_drop_1
)
'''
...
plt.legend(loc='best')
plt.savefig('/home/cemoser/Repositories/ostrich/Tests/Figures/emulator3D/units_test/simba_'+prof+'_'+vary_str+'.png',bbox_inches='tight') 
plt.close()
#make these plots when we have chosen a kernel for each feedback param

#radial error plots
for i in np.arange(nsamp-760):
    a,b,c=fs.deconstruct_3D(i) #a is the ASN1 index, b is the z index, c is the mass_index 
    newy=y[:,i]
    newerrup=errup[:,i]
    newerrlow=errlow[:,i]
    ...
    plt.xlabel('R (Mpc)',size=12)
    plt.legend(loc='best',fontsize=8)
    plt.savefig('/home/cemoser/Repositories/ostrich/Tests/Figures/emulator3D/units_test/errs_'+suite+'_'+prof+'_'+vary_str+'_'+str(vary[a])+'_z'+str(z[b])+'_M'+mass_str[c]+'.png',bbox_inches='tight')
    plt.close()
